...
//     final url = Uri.parse("http://192.168.0.24:5000/api/auth/register");
...
class _SignupState extends State<Signup> {
  ...
  Future<void> registerUser() async {
    ...
    final url = Uri.parse("http://192.168.0.60:5000/api/auth/register");
    ...
    try {
      ...
      if (response.statusCode == 201) {
        ScaffoldMessenger.of(context).showSnackBar(
          SnackBar(content: Text(responseBody["msg"] ?? "Registration successful!")),
        );
        ...
        ScaffoldMessenger.of(context).showSnackBar(
          SnackBar(content: Text(responseBody["msg"] ?? "Registration failed!")),
        );
      }
    } catch (e) {
      setState(() {
