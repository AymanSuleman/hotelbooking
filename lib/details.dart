...
class _HotelDetailState extends State<HotelDetail>
  ...
  Future<void> fetchRoomDetails() async {
    final url = Uri.parse('http://192.168.0.39:5000/api/rooms/${widget.roomId}');
    try {
      final response = await http.get(url);
      if (response.statusCode == 200) {
        setState(() {
  ...
  Future<void> checkIfWishlisted() async {
    final url = Uri.parse('http://192.168.0.39:5000/api/wishlist/${widget.userId}');
    try {
      final response = await http.get(url);
      if (response.statusCode == 200) {
        final wishlist = json.decode(response.body) as List;
        setState(() {
          isWishlisted = wishlist.any((item) => item['roomId']['_id'] == widget.roomId);
        });
      }
    } catch (e) {
      print("Wishlist check failed: $e");
    }
  }
  ...
  Future<void> toggleWishlist() async {
    ...
    try {
      ...
      if (response.statusCode == 200 || response.statusCode == 201) {
        ...
        print("Failed to toggle wishlist: ${response.body}");
      }
    } catch (e) {
          ...
          children: [
            ...
            Padding(
              ...
              child: Column(
                crossAxisAlignment: CrossAxisAlignment.start,
                children: [
                  Text(room!.name, style: TextStyle(fontSize: 22, fontWeight: FontWeight.bold)),
                  SizedBox(height: 5),
                  Row(
                    children: [
                      Icon(Icons.location_on, color: Colors.blue),
                      SizedBox(width: 5),
                      Expanded(child: Text(room!.location, style: TextStyle(color: Colors.grey))),
                    ],
                  ),
                  SizedBox(height: 10),
                  Row(
                    ...
                    children: [
                      ...
                      Container(
                        ...
                        child: Text("10% Off", style: TextStyle(fontWeight: FontWeight.bold, color: Colors.blue)),
                      ),
                    ],
                  ),
                ],
            ...
            TabBar(
              ...
              labelColor: Colors.black,
              unselectedLabelColor: Colors.grey,
              tabs: [Tab(text: "About"), Tab(text: "Gallery"), Tab(text: "Review")],
            ),
            Container(
              height: 400,
              child: TabBarView(
            ...
            child: Row(
              ...
              children: [
                Column(
                  ...
                  children: [
                    Text("Total Price", style: TextStyle(color: Colors.grey)),
                    Text("\$${room!.price}/night", style: TextStyle(fontSize: 18, color: Colors.blue, fontWeight: FontWeight.bold)),
                  ],
                ),
                ElevatedButton(
                  onPressed: () => showGuestSelectionSheet(context),
  ...
  Widget _buildAboutSection(String aboutText) {
    ...
    return Padding(
      ...
      child: Column(
        ...
        children: [
          Text(
            _isExpanded || !isTextLong ? aboutText : aboutText.substring(0, 200) + "...",
            style: TextStyle(fontSize: 16),
          ),
          SizedBox(height: 10),
          if (isTextLong)
            InkWell(
              onTap: () => setState(() => _isExpanded = !_isExpanded),
              child: Text(_isExpanded ? "Read Less" : "Read More", style: TextStyle(color: Colors.blue, fontWeight: FontWeight.bold)),
            ),
        ],
      ),
    );
...
class _GuestSelectionBottomSheetState extends State<GuestSelectionBottomSheet> {
  ...
  Widget build(BuildContext context) {
    return Padding(
      ...
      child: Column(
        mainAxisSize: MainAxisSize.min,
        children: [
          Text("Select Guests", style: TextStyle(fontSize: 18, fontWeight: FontWeight.bold)),
          SizedBox(height: 10),
          buildCounter("Adults", adults),
          buildCounter("Children", children),
          buildCounter("Infants", infants),
  ...
  Widget buildCounter(String label, int count) {
    return Row(
      mainAxisAlignment: MainAxisAlignment.spaceBetween,
      children: [
        Column(crossAxisAlignment: CrossAxisAlignment.start, children: [Text(label), Text("Age details")]),
        Row(
          children: [
            IconButton(icon: Icon(Icons.remove_circle, color: Colors.blue), onPressed: () => updateGuest(label, false)),
            Text('$count'),
            IconButton(icon: Icon(Icons.add_circle, color: Colors.blue), onPressed: () => updateGuest(label, true)),
          ],
        )
      ],
    );
